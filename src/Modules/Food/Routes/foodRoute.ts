...
import {all, create, darEntrada, getById, registrarSaida, relatorioEntrada, relatorioSaida, remove, update} from '../Repositories/foodRepository'
import { z } from 'zod'
...
export async function foodRoutes(app: FastifyInstance) {
  ...
  app.post('/entrada', async (request, reply) => {
	try {
	  ...
	  return reply.status(201).send({ message: 'Entrada registrada com sucesso' })
	} catch (error) {
		console.error(error)
